...
Dokumentacja HTML Living Standard (https://html.spec.whatwg.org/) dostarcza zestawu sugestii dotyczących renderowania dokumentów HTML, których przestrzeganie jest prawdopodobnie zgodne z intencjami deweloperów.
...
-
typu reset, którego rolą jest zresetowanie elementów formularza do stanu początkowego m.in. "wyczyszczenie" pól tekstowych, zresetowanie zaznaczeń pól wyboru; obecnie nie poleca się stosowania tego przycisku, zwłaszcza gdyby mógł zostać pomylony z wyślij.
1.
...
5.
* Żeby lepiej zrozumieć działanie metody post utwórz następujący plik w języku programowania Python (interpreter tego języka musi być również zainstalowany) i zapisz go jako app.py:

  from flask import Flask, request app = Flask(__name__) @app.route("/", methods=['POST']) def hello_world(): return "I received email:" + request.form['email'] + ' and content: ' + request.form['content']
...
Wielokrotny wybór dla danej listy można ustawić za pomocą atrybutu multiple po którym należy użyć atrybutu size przyjmujący wartość liczbową będącą liczbą opcji do wybrania. Wiele opcji w większości przeglądarek można wybrać przytrzymując klawisz ctrl i klikając na opcje, które chcemy wybrać.
...
4.
color (preferowany kolor motywu)
...
3.
search (wyszukiwanie miasta)
4.
range (liczba biletów od 1 do maksymalnie 10)
Zadanie 8
...
Korzystając z dokumentacji dostępnej na https://developer.mozilla.org/en-US/docs/Web/HTML/Element/input (również z zakładek validation dla danego typu input) ustal następujące wymagania:
-
...
W przeglądarce Google Chrome zainstaluj rozszerzenie: https://chrome.google.com/webstore/detail/screen-reader/kgejglhpjiefppelpmljglcjbhoiplfn?utm_source=ext_sidebar&hl=pl
